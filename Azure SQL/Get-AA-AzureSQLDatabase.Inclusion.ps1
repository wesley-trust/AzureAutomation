...
#Script name: Get-AA-AzureSQLDatabase-Inclusion
...
.Synopsis
    A script that emails a list of SQL databases on a specific server, optionally including specified elastic pools.
.DESCRIPTION
    A script that gets SQL databases on a specific server, including only databases in any specified pools,
...
try {
    ...
    if ($SQLDatabases){
        ...
        $Subject =  "Databases in an Elastic Pool on SQL Server $SQLServer"
